...
class NestSight:
    ...
    def detect_occupancy(self, frame):
        ...
        if min(diff_empty, diff_birdie) > OCCUPANCY_MATCH_THRESHOLD:
            # Looks like neither reference -> something unexpected in the module
            state = BirdieState.ERROR
        elif diff_empty <= diff_birdie:
            state = BirdieState.EMPTY
        else:
            state = BirdieState.BIRDIE

        print(f"[OCCUPANCY] diff_empty={diff_empty:.1f} diff_birdie={diff_birdie:.1f} "
              f"threshold={OCCUPANCY_MATCH_THRESHOLD} state={state.name}")
        return state
